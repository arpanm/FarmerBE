...
public class CollectionCenterResource {
    ...
    public ResponseEntity<List<CollectionCenterDTO>> getAllCollectionCenters(
        ...
        if ("demanddata-is-null".equals(filter)) {
            LOG.debug("REST request to get all CollectionCenters where demandData is null");
            return new ResponseEntity<>(collectionCenterService.findAllWhereDemandDataIsNull(), HttpStatus.OK);
        }
