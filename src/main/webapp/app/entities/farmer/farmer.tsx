...
import { getEntities, reset } from './farmer.reducer';
...
export const Farmer = () => {
  ...
  const links = useAppSelector(state => state.farmer.links);
  ...
  return (
    <div>
      <h2 id="farmer-heading" data-cy="FarmerHeading">
        <Translate contentKey="farmerBeApp.farmer.home.title">Farmers</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="farmerBeApp.farmer.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/farmer/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="farmerBeApp.farmer.home.createLabel">Create new Farmer</Translate>
          </Link>
        </div>
      </h2>
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={farmerList ? farmerList.length : 0}
          next={handleLoadMore}
          ...
          {farmerList && farmerList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="farmerBeApp.farmer.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                  </th>
              ...
              <tbody>
                {farmerList.map((farmer, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{farmer.isActive ? 'true' : 'false'}</td>
                    <td>{farmer.createddBy}</td>
                    <td>{farmer.createdTime ? <TextFormat type="date" value={farmer.createdTime} format={APP_DATE_FORMAT} /> : null}</td>
                    <td>{farmer.updatedBy}</td>
                    <td>{farmer.updatedTime ? <TextFormat type="date" value={farmer.updatedTime} format={APP_DATE_FORMAT} /> : null}</td>
                    <td className="text-end">
                      <div className="btn-group flex-btn-group-container">
                        ...
                        <Button tag={Link} to={`/farmer/${farmer.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
                        ...
                        <Button
                          onClick={() => (window.location.href = `/farmer/${farmer.id}/delete`)}
                          color="danger"
                          size="sm"
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="farmerBeApp.farmer.home.notFound">No Farmers found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
