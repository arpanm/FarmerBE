...
import { getEntities, reset } from './crop.reducer';
...
export const Crop = () => {
  ...
  const links = useAppSelector(state => state.crop.links);
  ...
  return (
    <div>
      <h2 id="crop-heading" data-cy="CropHeading">
        <Translate contentKey="farmerBeApp.crop.home.title">Crops</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="farmerBeApp.crop.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/crop/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="farmerBeApp.crop.home.createLabel">Create new Crop</Translate>
          </Link>
        </div>
      </h2>
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={cropList ? cropList.length : 0}
          next={handleLoadMore}
          ...
          {cropList && cropList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="farmerBeApp.crop.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                  </th>
                  <th className="hand" onClick={sort('name')}>
                    <Translate contentKey="farmerBeApp.crop.name">Name</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('name')} />
                  </th>
                  ...
                  <th className="hand" onClick={sort('skuId')}>
                    <Translate contentKey="farmerBeApp.crop.skuId">Sku Id</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('skuId')} />
                  </th>
                  ...
                  <th>
                    <Translate contentKey="farmerBeApp.crop.category">Category</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {cropList.map((crop, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    <td>
                      <Button tag={Link} to={`/crop/${crop.id}`} color="link" size="sm">
                        {crop.id}
                      </Button>
                    </td>
                    <td>{crop.name}</td>
                    ...
                    <td>{crop.updatedBy}</td>
                    <td>{crop.updatedTime ? <TextFormat type="date" value={crop.updatedTime} format={APP_DATE_FORMAT} /> : null}</td>
                    <td>{crop.category ? <Link to={`/category/${crop.category.id}`}>{crop.category.id}</Link> : ''}</td>
                    <td className="text-end">
                      <div className="btn-group flex-btn-group-container">
                        <Button tag={Link} to={`/crop/${crop.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                          <FontAwesomeIcon icon="eye" />{' '}
                          <span className="d-none d-md-inline">
                            <Translate contentKey="entity.action.view">View</Translate>
                          </span>
                        </Button>
                        <Button tag={Link} to={`/crop/${crop.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
                        ...
                        <Button
                          onClick={() => (window.location.href = `/crop/${crop.id}/delete`)}
                          color="danger"
                          size="sm"
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="farmerBeApp.crop.home.notFound">No Crops found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
